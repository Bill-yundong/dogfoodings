    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <div className="bg-white rounded-2xl p-8 shadow-sm border border-slate-100">
          <h3 className="text-xl font-bold text-slate-800 mb-8">模拟参数</h3>
          ...
          <div className="mt-8 p-6 bg-slate-50 rounded-2xl border border-slate-100">
            <h4 className="font-bold text-slate-700 mb-4">当前土壤条件</h4>
            {selectedSample ? (
          ...
          >
            {isRunning ? (
              <span className="flex items-center justify-center gap-3">
                <span className="animate-spin text-lg">⏳</span> 模拟计算中...
              </span>
            ) : (
              '🚀 开始模拟'
            )}
        ...
        <div className="lg:col-span-3 space-y-8">
          {isRunning && (
            <div className="bg-white rounded-2xl p-8 shadow-sm border border-slate-100">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-xl font-bold text-slate-800">模拟进度</h3>
                <span className="text-3xl font-bold text-crop-600">{progress.toFixed(0)}%</span>
              </div>
          ...
          {result && (
            <>
              <div className="grid grid-cols-3 gap-6">
                <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-2xl p-8 border border-blue-200">
                  <div className="flex items-center gap-4 mb-4">
                    <div className="w-12 h-12 bg-blue-500 rounded-2xl flex items-center justify-center text-white font-bold text-lg">N</div>
                    <div>
                      <p className="text-sm text-blue-600 font-medium">总氮吸收</p>
                      <p className="text-3xl font-bold text-blue-800">{result.totalUptake.nitrogen.toFixed(2)}</p>
                    </div>
                  </div>
                ...
                <div className="bg-gradient-to-br from-crop-50 to-crop-100 rounded-2xl p-8 border border-crop-200">
                  <div className="flex items-center gap-4 mb-4">
                    <div className="w-12 h-12 bg-crop-500 rounded-2xl flex items-center justify-center text-white font-bold text-lg">P</div>
                    <div>
                      <p className="text-sm text-crop-600 font-medium">总磷吸收</p>
                      <p className="text-3xl font-bold text-crop-800">{result.totalUptake.phosphorus.toFixed(2)}</p>
                    </div>
                  </div>
                ...
                <div className="bg-gradient-to-br from-amber-50 to-amber-100 rounded-2xl p-8 border border-amber-200">
                  <div className="flex items-center gap-4 mb-4">
                    <div className="w-12 h-12 bg-amber-500 rounded-2xl flex items-center justify-center text-white font-bold text-lg">K</div>
                    <div>
                      <p className="text-sm text-amber-600 font-medium">总钾吸收</p>
                      <p className="text-3xl font-bold text-amber-800">{result.totalUptake.potassium.toFixed(2)}</p>
                    </div>
                  </div>
              ...
              <div className="bg-white rounded-2xl p-8 shadow-sm border border-slate-100">
                <h3 className="text-xl font-bold text-slate-800 mb-8">养分吸收过程曲线</h3>
                
                <div className="h-72 flex items-end justify-around gap-1 px-6">
                ...
                <div className="flex justify-between mt-6 text-xs text-slate-400 px-6">
                  <span>第 0 天</span>
                  <span>氮素吸收累积曲线</span>
              ...
              <div className="bg-white rounded-2xl p-8 shadow-sm border border-slate-100">
                <h3 className="text-xl font-bold text-slate-800 mb-8">养分浓度动态变化</h3>
                
                <div className="grid grid-cols-3 gap-8">
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      <span className="text-sm font-bold text-slate-600">氮素浓度</span>
                      <span className="text-sm text-blue-600 font-bold">
                        {result.nutrientConcentration.nitrogen[result.nutrientConcentration.nitrogen.length - 1].toFixed(1)} mg/kg
                      </span>
                    </div>
                    <div className="h-36 bg-slate-50 rounded-2xl flex items-end gap-0.5 p-4">
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      <span className="text-sm font-bold text-slate-600">磷素浓度</span>
                      <span className="text-sm text-crop-600 font-bold">
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      <span className="text-sm font-bold text-slate-600">钾素浓度</span>
              ...
              <div className="bg-gradient-to-br from-blue-50 to-crop-50 rounded-2xl p-8 border border-blue-100">
                <h3 className="text-xl font-bold text-slate-800 mb-6">📊 模拟结果分析</h3>
                <div className="grid grid-cols-2 gap-6">
                  <div className="p-6 bg-white rounded-2xl shadow-sm">
                    <p className="text-sm text-slate-500 mb-2 font-medium">氮素吸收效率</p>
                    <p className="text-2xl font-bold text-blue-700">
                      {((result.totalUptake.nitrogen / (selectedSample?.totalNitrogen || 1)) * 100).toFixed(1)}%
                    </p>
                  </div>
                  <div className="p-6 bg-white rounded-2xl shadow-sm">
                    <p className="text-sm text-slate-500 mb-2 font-medium">磷素吸收效率</p>
                    <p className="text-2xl font-bold text-crop-700">
                      {((result.totalUptake.phosphorus / (selectedSample?.availablePhosphorus || 1)) * 100).toFixed(1)}%
                    </p>
                  </div>
                  <div className="p-6 bg-white rounded-2xl shadow-sm">
                    <p className="text-sm text-slate-500 mb-2 font-medium">钾素吸收效率</p>
                    <p className="text-2xl font-bold text-amber-700">
                      {((result.totalUptake.potassium / (selectedSample?.availablePotassium || 1)) * 100).toFixed(1)}%
                    </p>
                  </div>
                  <div className="p-6 bg-white rounded-2xl shadow-sm">
                    <p className="text-sm text-slate-500 mb-2 font-medium">模拟计算步数</p>
          ...
          {!result && !isRunning && (
            <div className="bg-white rounded-2xl p-24 shadow-sm border border-slate-100 text-center">
              ...
              <h3 className="text-3xl font-bold text-slate-700 mb-4">异步反应输运模拟</h3>
