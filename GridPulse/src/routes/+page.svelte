<script lang="ts">
  ...
  let historyData: { time: number; frequency: number }[] = [];
  ...
  onMount(() => {
    const interval = setInterval(() => {
      ...
      historyData = [...historyData, { time: now, frequency: $systemStatus.currentFrequency }];
      
      if (historyData.length > maxHistoryPoints) {
        historyData = historyData.slice(-maxHistoryPoints);
      }
    }, 100);
...
<div class="space-y-6">
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
    <MetricCard
      ...
      unit="Hz"
      status={Math.abs($systemStatus.frequencyDeviation) < 0.1 ? 'normal' : Math.abs($systemStatus.frequencyDeviation) < 0.2 ? 'warning' : 'danger'}
    ...
    <MetricCard
      ...
      unit="%"
      status={$systemStatus.stabilityMargin > 0.3 ? 'normal' : $systemStatus.stabilityMargin > 0.15 ? 'warning' : 'danger'}
