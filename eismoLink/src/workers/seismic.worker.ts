...
function calculateConfidence(data: SeismicDataPoint[], pWaveIdx: number, sWaveIdx: number): number {
...
function generateSyntheticData(duration: number, sampleRate: number): SeismicDataPoint[] {
...
ctx.onmessage = (event: MessageEvent) => {
  ...
  switch (type) {
    case 'PREDICT_WAVES': {
      ...
      ctx.postMessage({ type: 'PREDICTION_RESULT', payload: prediction });
      break;
    }
    ...
    case 'GENERATE_SYNTHETIC': {
      ...
      ctx.postMessage({ type: 'SYNTHETIC_DATA', payload: data });
      break;
    }
    ...
    case 'PROCESS_STREAM': {
      ...
      ctx.postMessage({ type: 'STREAM_RESULT', payload: { prediction, chunk } });
      break;
    }
